...
async def load_missing_modules():
    ...
    modules_repo = getattr(config, "modules_repo", "The-MoonTg-project/custom_modules")

    try:
        async with (
            aiohttp.ClientSession() as session,
            session.get(
                f"https://raw.githubusercontent.com/{modules_repo}/{config.modules_repo_branch}/full.txt"
            ) as resp,
        ):
            f = await resp.text()
    except Exception:
    ...
    async with aiohttp.ClientSession() as session:
        for module_name in all_modules:
            ...
            if not os.path.exists(module_path) and module_name in modules_dict:
                url = f"https://raw.githubusercontent.com/{modules_repo}/{config.modules_repo_branch}/{modules_dict[module_name]}.py"
